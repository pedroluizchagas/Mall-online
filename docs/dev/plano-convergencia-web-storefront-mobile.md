...
> **Feito em 2026-09-22** (`1fcccc8`). `removerObjetosDoTenant` + `diffDeMidia` (puro, testado) cobrem galeria retirada, recorte trocado, foto principal substituída, logo/banner anteriores e fotos da casa removidas — sempre depois de a escrita dar certo. **Reconciliação escrita em 2026-09-22:** `supabase/functions/reconciliar-midia/` — dry-run por padrão, exige `service_role`, ignora objeto com menos de 24 h (pode ser upload em curso) e aceita `tenant_id` para rodar por lojista. **Não foi executada nem checada por tipos** (Deno não está instalado nesta máquina) e **não foi deployada**. Ao escrevê-la apareceu o risco de verdade: `product_variants.foto_url` e `service_staff.foto_url` moram nos mesmos buckets, e sem eles no levantamento a função chamaria de órfã uma foto em uso — a parte perigosa é a enumeração de quem referencia cada bucket, não a varredura. Antes de rodar com `apagar: true`, revalidar a lista contra o schema.
