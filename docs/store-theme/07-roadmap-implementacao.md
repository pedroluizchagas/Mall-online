...
- [x] `packages/lib/src/store-theme/`: `types.ts`, `presets.ts` (`ARQUETIPOS` — **21 presets** em 2026-09; eram 11 na Fase 0, [03 §3.3]), `mapping.ts` (`CATEGORIA_SLUG_TO_ARQUETIPO`, cobertura das 20 categorias), `contrast.ts` (WCAG), `resolve.ts` (`resolveTheme` + `normalizeThemeConfig` migração v1→v2), `to-css-vars.ts`. Exportado via `@mallevo/lib`. (`provider.tsx` foi removido em 2026-09-19: nenhum app o consumia — cada superfície tem o seu adaptador.)
...
- [x] Componentes do catálogo tematizam por classe semântica sem mudança de markup (`StoreHeader`, `ProductCard`, `MenuSection`). `ProductModal`/CTAs: idioma "selecionado/primário" (`bg-ink text-accent`, que quebra na maioria dos presets) migrado para `bg-accent text-accent-ink`; botões `bg-accent` usam `text-accent-ink`.
...
- [x] `lib/store-theme.tsx`: adaptador RN — `colorsFromTheme(theme)` mapeia `ThemeTokens`→forma de `consumerDesign.colors` (+`accentInk`); `StoreColorsProvider`/`useStoreColors` (contexto com **default = paleta Mallevo**; evoluiu para `StoreDesignProvider`/`useStoreDesign` na Fase 4).
- [x] `app/loja/[slug].tsx`: `select` passa a trazer `theme`; `colors = colorsFromTheme(loja.theme)`; árvore envolvida em `StoreColorsProvider` (hoje `StoreDesignProvider`); FAB usa `accentInk`.
...
- [x] `minha-loja-editor.tsx`: substitui `market/boutique/artesanal/neon` + 6 paletas pelos arquétipos (**21** hoje), **sugeridos pela categoria** (`getArquetipoSugestao` → recomendados + outros estilos), com cards de preview (nome, descrição, mood). Preview do "celular" reusa o subsistema existente, agora alimentado por `temaFromTokens(resolveTheme(config))` — a MESMA engine do storefront/app ("o que vejo é o que publico"). Override de **cor de destaque** via color picker (+ restaurar). Tipografia mostra a fonte do arquétipo.
- [x] `loja-vitrine.ts`: grava `StoreThemeConfig v2` (`{v:2, preset, color?:{accent}}`); valida preset (enum de `ARQUETIPOS`) e accent (hex). `accentInk` é derivado no `resolveTheme` (contraste WCAG) na renderização.
...
- [x] **Etapa de tema no onboarding** ([06 §6.1]): passo "Escolha o estilo da sua loja" (etapa 8/12) entre a confirmação da categoria e o nome — cards com mini-preview real (cores+forma via `ARQUETIPOS`/`RADIUS_STEPS_PX`), default sugerido pela categoria com badge "Recomendado". `onboard-tenant` valida o preset (whitelist espelhada dos 21 — manter em sincronia) e grava `stores.theme` no INSERT: **toda loja nova nasce vestida**. ⚠️ Deploy da function pendente (ver nota `dados_bancarios` no commit).
...
- **Saída:** lojista escolhe um dos estilos (21 hoje) (sugeridos por nicho) + cor, vê o preview fiel e publica `StoreThemeConfig v2` que o storefront (Fase 1) e o app (Fase 2) renderizam.
...
- [x] **Paletas curadas por arquétipo** ([02 §2.4]) — `palettes.ts` (@mallevo/lib): 2 peles de cor alternativas por arquétipo (**44** no total em 2026-09; `roast` e `raw` têm 3), aplicadas em camada no `resolveTheme` (preset → paleta → overrides). Persistência enxuta: `stores.theme.palette` guarda só o código; hex refináveis centralmente. Editor ganha seção "Paleta do estilo" (chips Original + alternativas); `publicarVitrine` valida contra o catálogo. Autoria garantida por teste: `accentInk`×`accent` e `ink`×`bg` com AA ≥ 4.5 em toda paleta E preset (o teste pegou e corrigiu 3 presets que reprovavam: soft accentInk → #2A2A2A, clinic accent → #0D7A6E, market accent → #22863A).
- [x] **Ajuste fino de raios contra as refs-âncora** — calibrado com evidência (histograma de `border-radius` extraído do HTML das refs, 2026-07): raw sharp ✅ (Rawline 85× `0`), editorial sharp ✅ (Marion 20× `0`), soft round ✅ (PetPals 40px), heritage soft+pill ✅ (Bistora 33× `100`). **Correção**: noir era `sharp` sem respaldo — Aurum usa 20–24px em peso (116×) e Esteem 40px → noir agora é `soft` (luxo arredondado suave). Artisan (Graft 10–40px, misto) mantido em `soft` por decisão conservadora.
- [ ] **(bloqueado por produto)** Mapeamento fino wizard→arquétipo ([06 §6.4]) — os `wizardSteps` dos DashboardTemplates existem como DADO mas **nenhuma UI os renderiza** e não há armazenamento de respostas; o refinamento (ex.: fashion+streetwear → `raw`) só faz sentido depois de o wizard de nicho existir. Mitigação atual: a etapa de estilo do onboarding já mostra as alternativas compatíveis como cards visuais — o lojista de streetwear vê e escolhe o Raw diretamente.

## Fase 5 — Convergência web · storefront · mobile (2026-09) 🟡 EM ANDAMENTO
As vitrines por arquétipo (15, [05 §5.6]) nasceram só no consumer; o storefront ficou na pele e o editor do lojista não sabe que elas existem. O plano de fechar essa lacuna está em `docs/dev/plano-convergencia-web-storefront-mobile.md`. Fase 0 (2026-09-19): `VITRINES`/`resolveVitrine` em `store-theme/vitrines.ts` (tabela única do gate, consumida pelo consumer), `loja/horarios.ts` (`abertoAgora`/`horarioDeHoje`), `loja/conteudo.ts` (`stores.conteudo` v1: campanha/manifesto/galeria_casa/destaques) e `catalogo/metadata-produto.ts` (schema único de `products.metadata`, incluindo `galeria`/`recorte`/`especificacoes`/`unidade`); migration `20260919120000_stores_conteudo.sql`.
