# Stage 0 — Backend do Explorar (BLOQUEANTE do pilar Conteúdo)

> Nada de UI de publicação antes deste stage fechar e mergear. O app não tem
> pra onde publicar enquanto a tabela, o bucket e a view não existirem. Este
> stage também publica o **contrato de dado** que o Stage 9 vai consumir sem
> renegociar. O pilar Gestão (stages 3–6) **não** depende deste stage.

## Entregáveis

1. Migration `store_posts` (tabela + índices + RLS).
2. Migration bucket `explore-media` + policies de Storage.
...
## 1. Tabela `store_posts`

`supabase/migrations/<ts>_partner_01_store_posts.sql`:

```sql
-- ============================================================
-- Partner App Stage 0 — Fundação do Explorar (posts das lojas)
-- Fotos e vídeos no feed vertical do consumer (estilo Reels).
...
CREATE TABLE IF NOT EXISTS store_posts (
  ...
  media_path    TEXT NOT NULL,            -- fonte da verdade: {tenant_id}/{store_id}/{uuid}.(mp4|jpg)
  media_url     TEXT NOT NULL,            -- URL pública derivada (conveniência de leitura)
...
## 2. Bucket `explore-media`

`supabase/migrations/<ts>_partner_02_explore_media_bucket.sql` — mesma
...
CREATE POLICY explore_media_insert_proprio
...
CREATE POLICY explore_media_update_proprio
  ON storage.objects FOR UPDATE TO authenticated
  USING (bucket_id = 'explore-media'
         AND (storage.foldername(name))[1] = my_tenant_id()::text)
  WITH CHECK (bucket_id = 'explore-media'
         AND (storage.foldername(name))[1] = my_tenant_id()::text);

CREATE POLICY explore_media_delete_proprio
...
verificado via trigger"). Espelhar para `store_posts`: função
`check_store_posts_limit()` que conta posts `status <> 'removed'` do tenant e
compara com um teto do plano (`plans.max_posts`, nova coluna nullable; `NULL`
...
Mesma decisão do storefront: `anon` **não** toca `store_posts`. Lê uma
view que entrega o formato que o Explorar do consumer consome:
...
as outras tabelas) para que `store_posts` e `public_explore_feed` apareçam em
`Database`. Tanto `apps/mobile-partner` quanto `apps/mobile-consumer` consomem
via `createClient<Database>`.

## 6. Contrato (fonte da verdade do Stage 9)

`public_explore_feed` cobre o `interface Reel` de
[`explorar.tsx`](../../apps/mobile-consumer/app/(tabs)/explorar.tsx) — de
propósito — e o **estende** para fotos. O Stage 9 troca o array mock por um
`select` na view e adapta o `interface` local. Diferenças intencionais a
anotar para o Stage 9:

| `interface Reel` (mock) | `public_explore_feed` | Nota |
|---|---|---|
| `id: string` | `id uuid` | ok |
| — | `tipo 'video'\|'foto'` | **novo**: `ReelItem` renderiza `VideoView` ou `Image` |
| `loja_slug/nome/inicial` | colunas da view | derivado de `stores` |
| `video_url` | `media_url` | **renomeado** (posts de foto não são vídeo) |
| — | `thumb_url` | **novo**: usar na `GaleriaGrid` (hoje placeholder) |
| — | `duracao_seg` | vídeo apenas; foto = NULL |
...
- [ ] `store_posts` criada com RLS; INSERT só do próprio tenant+loja.
- [ ] `tipo='foto'` com `duracao_seg` preenchida é rejeitado (constraint).
- [ ] Bucket `explore-media` com cap 50 MB e mime allowlist (vídeo + imagem);
      escrita só no prefixo `{tenant_id}/`, leitura pública.
