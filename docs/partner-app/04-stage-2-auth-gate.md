...
da loja ativa para prefixar uploads e setar `store_videos.store_id`.
...
Antes de liberar a aba "Publicar", checar o estado do tenant. O projeto já
migrou o gate de assinatura para `pagarme_onboarding_status` (commit `e172f80`
"migrar gate para pagarme_onboarding_status"). **Reusar o mesmo predicado**, não
inventar outro:

- Centralizar `tenantPodePublicar(tenant)` em `packages/lib` (junto das outras
  regras de negócio compartilhadas). Espelha a lógica já aplicada no
  `create-subscription`/courier.
- Estados e tela:
...
| Tenant existe, gate reprova (`onboarding_status` não utilizável) | tela "Ative sua loja para publicar" + link Dashboard; abas read-only |
...
Tenant multi-loja: seletor simples (bottom sheet/lista) no header da aba
"Publicar" e em "Perfil". Troca `lojaAtivaId` (persistido). Todo upload e todo
registro `store_videos` usam `lojaAtivaId` → define `store_id` e o prefixo
`{tenant_id}/{store_id}/` no Storage.
...
- [ ] Tenant reprovado no gate vê tela de bloqueio com CTA para o Dashboard;
      não acessa "Publicar".
- [ ] Tenant multi-loja troca a loja ativa e a seleção persiste.
- [ ] `tenantPodePublicar` vive em `packages/lib` e replica o predicado atual
      (não há regra de gate duplicada no app).
