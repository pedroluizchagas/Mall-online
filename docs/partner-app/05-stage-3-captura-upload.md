# Stage 3 — Captura, Compressão e Upload

> O coração do produto: gravar/escolher → comprimir → revisar → legenda +
> produto → publicar, sem sair do celular. Depende dos Stages 0–2.

## Fluxo (uma tela com passos, não wizard longo)

```
[Permissões] -> [Capturar | Galeria] -> [Preview + trim] -> [Detalhes] -> [Upload] -> [Publicado]
```
...
- **Gravar**: `CameraView` (expo-camera) vertical fullscreen — mesma lente
  visual do Explorar do consumer (base dark). Botão rec, timer, **corte
  automático em 60 s** (limite do schema: `duracao_seg BETWEEN 1 AND 120`,
  alvo de produto 60 s), front/back, flash.
- **Galeria**: `expo-image-picker` (`mediaTypes: Videos`,
  `videoMaxDuration: 60`, permite trim nativo onde houver).

Saída dos dois caminhos: um `uri` local de vídeo.

### 3. Compressão (client-side)

`react-native-compressor` antes de qualquer upload:
...
- Preview com `expo-video` (loop, mute toggle) — espelha o `ReelItem` do
  consumer para o lojista ver "como vai aparecer".
- Campos:
...
### 5. Upload resumível (TUS)

Caminho dos objetos (case-sensível, exatamente o prefixo das policies):

```
explore-videos/{tenant_id}/{store_id}/{uuid}.mp4
explore-videos/{tenant_id}/{store_id}/{uuid}.jpg
```

Usar o upload resumível do `supabase-js` (protocolo TUS), não `upload()`
multipart:

- `useUploadStore` (Zustand) controla 1 upload por vez: estado
  (`comprimindo | enviando | criando-registro | concluido | erro`), `progress`
  0–1, ação `cancelar`, ação `tentarNovamente` (retoma do offset, não do zero).
...
- Vídeo primeiro, thumb depois. Só seguir para o passo 6 com os **dois**
  objetos confirmados (HTTP ok do Storage).

### 6. Criar registro `store_videos`

Após os dois objetos confirmados, `insert` em `store_videos` (RLS
`store_videos_insert_proprio` valida tenant+loja):

```ts
await supabase.from('store_videos').insert({
  store_id: lojaAtivaId,
  tenant_id: tenant.id,
  product_id: produtoSelecionado?.id ?? null,
  storage_path: videoPath,
  video_url: pub(videoPath),          // getPublicUrl
...
- `status`: `published` direto **se** a decisão de moderação do Stage 0 for
  `approved` por padrão; senão `processing`/`pending` e o vídeo aparece em
  "Meus vídeos" como "em análise". Não duplicar a decisão — ler o que o Stage 0
  fechou.
- **Atomicidade prática**: se o `insert` falhar após o upload, o objeto fica
  órfão. Mitigar: (a) registro logo após confirmação; (b) Stage 4 detecta
  objeto sem registro e oferece "retomar publicação" / limpeza; (c) job de
  limpeza de órfãos > 24 h é item de roadmap, não MVP.

### 7. Sucesso

Confirmação + atalho "Ver em Meus vídeos" (Stage 4) e "Publicar outro".

## Considerações

- **Sem transcode server-side** (decisão §5 do `01`). Toda normalização é a
...
- **Tamanho**: cap de 50 MB no bucket é a rede de segurança; o alvo real de
  ...
  bloquear o lojista.
- **Offline**: sem conexão, permitir gravar/comprimir e enfileirar; enviar
  quando voltar (estado persistido em `useUploadStore`). Fila multi-item é
  pós-MVP; MVP garante 1 upload robusto.

## Critérios de aceite

- [ ] Gravar 60 s e escolher da galeria geram o mesmo `uri` tratável.
- [ ] Compressão produz .mp4 H.264/AAC < 50 MB com progresso visível.
- [ ] Thumbnail .jpg gerado e enviado junto.
- [ ] Upload resumível: matar a rede no meio e retomar conclui sem reiniciar.
- [ ] Objetos no prefixo `{tenant_id}/{store_id}/`; RLS rejeita prefixo de
      outro tenant (teste negativo).
- [ ] Registro `store_videos` criado e visível na view `public_explore_feed`.
- [ ] Vídeo publicado **toca no `apps/mobile-consumer`** via `expo-video`
      (validação de codec real).
